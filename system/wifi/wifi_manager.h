 ...
 *
 * State machine:
 *   wifi_manager_init()      → subsystem ready (radio OFF)
 *   wifi_start()             → radio ON
 ...
 *   wifi_manager_deinit()    → full teardown
 *
 ...
 *  resources.  wifi_manager_init() must be called again before any other use. */
...
/** Turn WiFi radio ON.  Must call wifi_manager_init() first.  Safe to call when
 *  already on (no-op). */
 ...
 *  Must call wifi_manager_init() first. */
...
/** Read the most-recently-used saved credentials.  Reads a RAM cache that
 *  wifi_manager_init() primes from wifi_store (FlashDB), so it survives
 *  a reboot.  For the full saved list, use wifi_store.h directly. */
...
/** Forget one saved network (removes it from wifi_store and disconnects if
 *  that's the one currently connected).  Needed to change a wrong password:
 *  without it, auto-connect keeps retrying the bad one. */
