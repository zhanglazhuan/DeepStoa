...
typedef struct {
	...
	int stack[10];
	int stack_top;
	int current_page;              // track current page for auto-push
	void *nav_ctx;
	app_handle_t app_handler;      // for navigate_back callback
} page_navigator_t;
...
// 跳转到指定页面 (自动 push 当前页到返回栈，兼容 EPOS 3 参数调用)
// 用法: PAGE_NAVIGATE_TO(app_ptr, TO_PAGE, user_data)
#define PAGE_NAVIGATE_TO(app_ptr, to, data) \
	do { \
		page_navigator_push(&(app_ptr)->view->page_nav, \
		                    (app_ptr)->view->page_nav.current_page); \
		page_navigator_navigate_to(&(app_ptr)->view->page_nav, (app_ptr), (to), (data)); \
		(app_ptr)->view->page_nav.current_page = (to); \
	} while(0)
