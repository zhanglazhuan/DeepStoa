...
/* PC 上的 server/run.py —— news / ota / logs 同一个端口，
 * 192.168.137.1 是 Windows 移动热点的固定网关。详见 server/README.md。 */
#define PODCAST_SERVER      "http://192.168.137.1:8000"
...
static int upload_chunk(const char *session_base, int start_line)
{
    ...
    if (off < 0 || off >= 8192) { free(payload); return -1; }

    for (int i = 0; i < line_count; i++) {
        if (off + 3 >= 8192) { free(payload); return -1; }
        if (i > 0) payload[off++] = ',';
        payload[off++] = '"';
        for (const char *p = lines[i]; *p; ++p) {
            ...
            if (esc) {
                size_t n = strlen(esc);
                if (off + n >= 8192) { free(payload); return -1; }
                memcpy(payload + off, esc, n); off += n;
            } else {
                one[0] = *p;
                if (off + 1 >= 8192) { free(payload); return -1; }
                payload[off++] = one[0];
            }
        }
        if (off + 1 >= 8192) { free(payload); return -1; }
    ...
    if (off + 3 >= 8192) { free(payload); return -1; }
    payload[off++] = ']'; payload[off++] = '}'; payload[off] = '\0';
